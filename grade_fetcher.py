...
class GradeFetcher:
    ...
    def save_history(self, grades_data):
        if not self.config.get('save_to_file', True):
            return
        data = {'last_update': self.last_update_time, 'history': self.grade_history[-50:], 'last_check': time.strftime('%Y-%m-%d %H:%M:%S')}
        try:
    ...
    def check_for_updates(self, grades_data):
        ...
        self.grade_history.append({'check_time': check_time, 'latest_update': latest_time, 'has_new': latest_time != self.last_update_time and self.last_update_time != ""})
        if self.last_update_time and latest_time > self.last_update_time:
            ...
            return True, {'new_update_time': latest_time, 'old_update_time': old_time, 'new_courses': new_courses, 'total_data': grades_data}
            ...
            for term in grades_data['data'].get('term', []):
                for course in term.get('creditInfo', []):
                    ...
                    if update_time and update_time > self.last_update_time:
                        new_courses.append({'course_name': course.get('courseName', ''), 'score': course.get('score', ''), 'update_time': update_time, 'term': term.get('termName', '')})
        except Exception:
            pass
        return new_courses
