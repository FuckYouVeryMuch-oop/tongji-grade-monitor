...
> 学号获取：登录 1.tongji.edu.cn 后，在成绩页面 URL 或页面源码中可找到 studentId 参数，即为你的学号。
...
双击 **点我—后台运行.bat** 即可在后台运行（使用 pythonw.exe，无控制台窗口）。如果已经有一个实例在运行，会提示并自动退出。首次使用建议先前台运行确认配置正确。
...
tongji-grade-monitor/
├── main.py                    # 主程序入口
├── 点我—后台运行.bat          # 双击 后台运行（推荐）
├── 点我—前台运行.bat          # 双击 前台看控制台
├── start_bg.bat               # 被后台.bat 调用
